...
A Python-based test client is provided in the `test-cli-client` directory. This client connects to the server, displays the game state in the console, and allows you to observe the BOTs in action. For fully automated multi-client testing, see the Go integration harness in `server/integration_test.go` (`go test -run TestHTTP`).
